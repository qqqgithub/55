...
Download_SSR(){
	...
	env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/qqqgithub/shadowsocksr.git
...
Write_Service_Centos(){
		if ! wget --no-check-certificate https://raw.githubusercontent.com/qqqgithub/Bash/master/init/centos-ssr -O /etc/init.d/${Service_ID}; then
...
Write_Service_Debian(){
		if ! wget --no-check-certificate https://raw.githubusercontent.com/qqqgithub/Bash/master/init/debian-ssr -O /etc/init.d/${Service_ID}; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		Write_Dir
		sed -i 's/NAME="AShadowsocksR"/NAME="'${Service_ID}'-ShadowsocksR"/' /etc/init.d/${Service_ID}
